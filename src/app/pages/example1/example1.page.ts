...
export class Example1Page implements OnInit {
  ...
  ngOnInit(): void {
    // create the editor
    const container = document.getElementById('jsoneditor1');
    const options = {
      mode: 'code',
    ...
    this._metadataSvc.getMetadata('./assets/json/response1.json', 'metaData1').subscribe((res:any)=>{
      this.metadata = res;
  ...
  save(){
   this._metadataSvc.setMetadata(this.metadata, 'metaData1');
  }
}
